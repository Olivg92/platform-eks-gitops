...
slo_rules() {
  groups=$(kubectl get prometheusrule -n demo demo-api -o jsonpath='{.spec.groups[*].name}' 2>/dev/null | wc -w)
  [ "$groups" -ge 6 ] && echo "$groups rule groups generated by Sloth" || { echo "no SLO rules"; return 1; }
}

slo_dashboard() {
  cm=$(kubectl get cm -n demo -l grafana_dashboard=1 -o name 2>/dev/null | wc -l)
  [ "$cm" -ge 1 ] && echo "dashboard shipped with the application" || { echo "no dashboard ConfigMap"; return 1; }
}
...
check "SLO rules from Sloth"          slo_rules
check "SLO dashboard in Grafana"      slo_dashboard
