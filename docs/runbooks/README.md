...
The `kubectl` commands below assume `KUBECONFIG` points at the environment being debugged, for
example `export KUBECONFIG=~/.kube/platform-eks-gitops-local`. The `make` targets set it themselves.
